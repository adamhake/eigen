# Part 29: Module Federation — Micro-Frontends with Vite
...
```
┌─────────────────────────────────────────────────┐
│  Host Application (Eigen framework, main shell)   │
│                                                   │
│  ┌──────────────┐                                │
│  │ Navigation   │ (Host's own component)         │
│  └──────────────┘                                │
│                                                   │
│  ┌──────────────────┐  ┌──────────────────────┐  │
│  │ Remote: Dashboard │  │ Remote: Marketplace  │  │
│  │ (Team A's build)  │  │ (Team B's build)     │  │
│  │ cdn.a.com/rem.js  │  │ cdn.b.com/rem.js     │  │
│  └──────────────────┘  └──────────────────────┘  │
│                                                   │
│  ┌─────────────────────────────────────────────┐ │
│  │ Remote: Checkout (Team C's build)            │ │
...
```typescript
// Remote application (dashboard) — vite.config.ts
import { defineConfig } from 'vite'
import react from '@vitejs/plugin-react'
import { federation } from '@module-federation/vite'
...
The `singleton: true` flag is critical for React — two React instances cause hooks to break. The version range ensures compatibility while allowing minor version differences.
