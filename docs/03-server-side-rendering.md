# Part 3: Server-Side Rendering — The SSR Environment
...
**Concepts introduced:** Vite's SSR mode, `server.ssrLoadModule`, `createServer` programmatic API, `transformIndexHtml`, dual module graphs, typing `ssrLoadModule` results.
...
Notice the `matchRoute` function appears in both the server and client entries. This duplication is deliberate — the route matching logic must be identical on both sides to ensure hydration consistency. A production framework would extract this into a shared module that both entries import.
