...
1. **Detect** files that use `createServerFn`
2. **Extract** each server function (its variable name, its argument type, its body)
3. **Register** each function as an API endpoint (with a unique ID)
4. **Replace** the function body in the client bundle with a `fetch()` call
5. **Leave** the function body intact in the server bundle

```typescript
// plugins/eigen-server-fns.ts
...
```tsx
// src/pages/Dashboard.tsx (ORIGINAL SOURCE)
import { createServerFn } from 'eigen/server'
...
```tsx
// The SSR environment gets the original code — createServerFn is a pass-through
import { createServerFn } from 'eigen/server'
import { db } from '../lib/database'
...
```typescript
// packages/eigen/server.ts — builder pattern
