...
### The [`config`](https://vite.dev/guide/api-plugin#config) hook
...
These defaults can be overridden by the user's config. Vite merges configs with a deep merge strategy, so the user can override `build.rolldownOptions.input` without losing the `manifest: true` setting.

<TypeDeepDive title="Concept: Deep merge vs. shallow merge">

A shallow merge (`Object.assign` or `{ ...a, ...b }`) only merges top-level properties — if both objects have a `build` key, `b.build` completely replaces `a.build`. A deep merge recurses into nested objects, so `a.build.manifest` and `b.build.rolldownOptions` both survive. Vite uses deep merge for plugin configs, which is why a framework can set `manifest: true` and the user can set `rolldownOptions.input` without either overwriting the other. Arrays are typically replaced, not concatenated — so if two plugins both set `build.rolldownOptions.external`, the last one wins.

</TypeDeepDive>

### The [`buildApp`](https://vite.dev/guide/api-plugin#buildapp) hook
...
**Before `buildApp`:** Frameworks either ran separate `vite build` commands via npm scripts (`build:client && build:server`), or used tools like [Vinxi](https://vinxi.vercel.app) that spawned multiple Vite instances. The problem with separate commands: the builds can't share state. The client build might discover routes that the server build doesn't know about. A shared virtual module has to generate its code independently in each build, potentially producing inconsistent results.
...
buildApp: {
	...
	async handler(builder) {
		...
		// Each has its own build config (outDir, rolldownOptions, etc.)
...
This three-layer pattern is universal across typed frameworks. [TanStack Router](https://tanstack.com/router) has static types (`RouteOptions`, `Link`), generated types (`routeTree.gen.ts`), and virtual modules. Next.js has static types (`GetServerSideProps`, `NextPage`), limited generated types (the `.next/types` directory), and internal virtual modules.

---

## Comparing to real frameworks

| Aspect | Eigen | [TanStack Start](https://tanstack.com/start) | [Next.js](https://nextjs.org) |
|---|---|---|---|
...
| SSR | `ssrLoadModule` + H3 | Nitro server | Built-in Node server |
...
Since Part 2, `Router`, `Link`, and `matchRoute` have lived inline in the app's entry file. Now that Eigen has a proper package structure, it's time to move them into the framework:
...
export function Link<T extends RoutePaths>({
	to,
	params,
	children,
}: LinkProps<T>) {
...
This is the same code from Part 2, just moved. `matchRoute` stays private — it's an implementation detail, not a public API. `Router` and `Link` are the framework's public runtime exports.
...
Now `entry-client.tsx` becomes thin:

```tsx title="src/entry-client.tsx"
import { hydrateRoot } from "react-dom/client";
import { Router, Link } from "eigen";
...
The entry point does what entry points should do: mount the app, nothing more. The routing logic lives in the framework.
...
- **[Part 12: Streaming SSR](/12-streaming-ssr)** — Replace `renderToString` with `renderToPipeableStream` for progressive page rendering powered by Suspense.
- **[Part 13: Nested Layouts](/13-nested-layouts)** — Implement a `layout.tsx` convention with recursive virtual module generation and typed parent-to-child data flow.
- **[Part 14: Framework Middleware](/14-framework-middleware)** — Typed context accumulation, `defineMiddleware`, `redirect()`, auth guards, the `createMiddlewareChain` builder pattern.
- **[Part 15: Server Functions](/15-server-functions)** — `createServerFn`, the `transform` hook generating RPC stubs, type preservation across transforms.
- **[Part 16: Static Site Generation](/16-static-site-generation)** — Pre-render pages at build time using the `closeBundle` hook, with a typed `generateStaticParams` convention.
- **[Part 17: Deployment Adapters](/17-deployment-adapters)** — The adapter pattern for hosting providers (Netlify, Cloudflare, Vercel, Node standalone).
- **[Part 18: Runtime Validation](/18-runtime-validation)** — Integrate [zod](https://zod.dev)/[valibot](https://valibot.dev) schemas to validate serialized data at the hydration boundary.
- **[Part 19: Edge Runtimes](/19-edge-runtimes)** — Add a third Vite environment targeting [Cloudflare Workers](https://developers.cloudflare.com/workers/) with Web API types.
- **[Part 21: React Server Components](/21-react-server-components)** — The capstone: three environments, `"use client"` boundaries, RSC streaming payloads, and `@vitejs/plugin-rsc`.

---

## Further reading

- [Vite Plugin API: `buildApp`](https://vite.dev/guide/api-plugin#buildapp) — The hook that gives frameworks control over multi-environment build orchestration.
- [Vite Plugin API: `config`](https://vite.dev/guide/api-plugin#config) — How plugins set framework defaults that merge with user configuration.
- [Vite Environment API](https://vite.dev/guide/api-environment) — The system that enables client, SSR, and edge environments within a single Vite instance.
- [TanStack Start](https://tanstack.com/start) — A production Vite-based framework that uses the same `buildApp` and plugin composition patterns.
- [TypeScript Declaration Files](https://www.typescriptlang.org/docs/handbook/declaration-files/introduction.html) — How `.d.ts` files bridge runtime code and the type system, the foundation of Layer 2 and Layer 3 types.
