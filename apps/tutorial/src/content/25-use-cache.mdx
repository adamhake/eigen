---
title: "Part 25: The use cache Directive"
description: "Compiler-driven caching with cacheLife and cacheTag semantics, cache key derivation, and stale-while-revalidate."
---
...
The developer uses two helper functions alongside the directive: `cacheLife` configures the TTL (how long the cached result is valid), and `cacheTag` associates the cached entry with a named tag for targeted invalidation. Both are imported from the framework and called inside the cached function body:
...
<Steps>
<Step>
**Derive cache key** from the function ID + serialized arguments
</Step>
<Step>
**Check the cache store** (in-memory, Redis, KV store, etc.)
</Step>
<Step>
**On hit:** return the cached value
</Step>
<Step>
**On miss:** call the original function, store the result, return it
</Step>
<Step>
**Respect `cacheLife` TTL** and `cacheTag` for revalidation
</Step>
</Steps>

### The transform plugin

```typescript title="plugins/eigen-cache.ts"
...
```typescript title="packages/eigen/runtime/cache.ts"
...
Like server functions (Part 15), cache keys require that arguments are JSON-serializable. You can't cache a function that receives a `Date`, a `Map`, or a React element as an argument — the cache key derivation would produce inconsistent results. This is the same serialization boundary that `"use server"` enforces.
...
```typescript title="eigen.config.ts"
export default {
  cacheHandlers: {
