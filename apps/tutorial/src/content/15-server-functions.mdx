...
export function generateFnId(file: string, name: string): string {
...
The `generateBundle` hook runs during the client build and can output additional files:
...
<TestSection title="Testing: server function transforms">

Server functions have two testable surfaces: the transform that generates RPC stubs, and the endpoint that executes them.

```typescript title="plugins/__tests__/eigen-server-fns.test.ts"
import { describe, it, expect } from 'vitest'
import { generateFnId } from '../eigen-server-fns'

describe('generateFnId', () => {
  it('produces stable IDs for the same input', () => {
    const id1 = generateFnId('/src/pages/Dashboard.tsx', 'getUsers')
    const id2 = generateFnId('/src/pages/Dashboard.tsx', 'getUsers')
    expect(id1).toBe(id2)
  })

  it('produces different IDs for different functions', () => {
    const id1 = generateFnId('/src/pages/Dashboard.tsx', 'getUsers')
    const id2 = generateFnId('/src/pages/Dashboard.tsx', 'deleteUser')
    expect(id1).not.toBe(id2)
  })

  it('produces different IDs for same name in different files', () => {
    const id1 = generateFnId('/src/pages/Dashboard.tsx', 'getData')
    const id2 = generateFnId('/src/pages/Settings.tsx', 'getData')
    expect(id1).not.toBe(id2)
  })
})

describe('client transform', () => {
  it('replaces createServerFn body with fetch stub', () => {
    const input = `import { createServerFn } from 'eigen/server'
import { db } from '../lib/database'
...
export default function Dashboard() {
  return <div>Dashboard</div>
    ...
    // The server-only code is gone
    expect(transformed).not.toContain('createServerFn')
    expect(transformed).not.toContain('db.query')
    expect(transformed).not.toContain("'eigen/server'")

    // The component survives
    expect(transformed).toContain('export default function Dashboard')
  })
})
```

The ID stability test is critical — if IDs change between server and client builds, the client's `fetch()` calls will hit wrong endpoints. The hash-based approach guarantees deterministic IDs from the same inputs.

</TestSection>

## What's next

In Part 16, we'll add **static site generation** — using the `generateBundle` hook to pre-render pages at build time, with a typed `generateStaticParams` convention for dynamic routes.
