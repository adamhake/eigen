...
When the dev server starts, Vite scans your imports and detects that `react` and `react-dom/client` need to be pre-bundled. It runs esbuild to convert them from CommonJS (React's default format) to ESM, combining each package's internal files into a single module. The results are cached in `node_modules/.vite/deps/`.
