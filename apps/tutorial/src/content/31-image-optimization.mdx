---
title: "Part 31: Image Optimization"
description: "Build-time image processing with sharp, responsive srcset generation, blur placeholders, and DAM integration."
---
...
**Concepts introduced:** The `load` hook for image processing, [`sharp`](https://sharp.pixelplumbing.com/) (a high-performance Node.js image processing library) for build-time image transforms, responsive `srcset` generation, WebP/AVIF format conversion, blur placeholder generation (LQIP — Low-Quality Image Placeholder), the `<Image>` component, `transformIndexHtml` for preload hints, DAM URL transformation for Cloudinary/Imgix, image CDN patterns, the `resolveId` hook for virtual image modules.

---

## The problem: images on the modern web

<Callout type="warn" title="The #1 web performance mistake">
Serving unoptimized images — a 4MB JPEG hero that's 4000px wide served to a 375px phone — wastes bandwidth, delays LCP, and tanks Core Web Vitals.
</Callout>
...
```typescript title="plugins/eigen-images.ts"
...
```typescript title="packages/eigen/image-providers.ts"
...
```tsx title="packages/eigen/image.tsx"
