...
Vite uses [chokidar](https://github.com/paulmillr/chokidar) (a file system watcher) to monitor source files. When a file is saved, chokidar fires an event.
</Step>
<Step>
### Module graph lookup
...
Vite walks *up* the dependency tree (from the changed module toward the root) looking for an **HMR boundary** — a module that has registered an [`import.meta.hot`](https://vite.dev/guide/api-hmr)`.accept()` handler. This handler says "I know how to update myself when my dependencies change."

[`@vitejs/plugin-react`](https://github.com/vitejs/vite-plugin-react) injects HMR boundary code into every React component via its `transform` hook (using [Oxc](https://oxc.rs) for the [React Fast Refresh](https://github.com/facebook/react/tree/main/packages/react-refresh) transform). This is why React components hot-reload by default — each component is a boundary.
</Step>
<Step>
### WebSocket update
...
## [`handleHotUpdate`](https://vite.dev/guide/api-plugin#handlehotupdate) — Custom HMR behavior
...
This creates a feedback loop: the developer adds a file, and within seconds, the IDE offers autocomplete for the new route's path and params. This is the same cycle that [TanStack Router](https://tanstack.com/router)'s `@tanstack/router-plugin/vite` implements when it watches the filesystem and regenerates `routeTree.gen.ts`.
...
- [Vite HMR API](https://vite.dev/guide/api-hmr) — The full reference for `import.meta.hot`, including `accept`, `dispose`, `invalidate`, and `prune`.
- [Vite Plugin API: `handleHotUpdate`](https://vite.dev/guide/api-plugin#handlehotupdate) — How plugins customize HMR behavior for framework-specific file changes.
- [React Fast Refresh](https://github.com/facebook/react/tree/main/packages/react-refresh) — React's hot reloading runtime that preserves component state across edits.
- [chokidar](https://github.com/paulmillr/chokidar) — The file system watcher library that Vite uses for change detection.
- [`@vitejs/plugin-react`](https://github.com/vitejs/vite-plugin-react) — The official React plugin for Vite, which injects HMR boundary code via the transform hook.
