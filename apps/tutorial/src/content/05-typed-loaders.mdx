...
The simplest approach — the developer explicitly writes the types. This is what you'd find in frameworks like [TanStack Start](https://tanstack.com/start) and [Remix](https://remix.run) before they invested in type inference:
...
We add a data endpoint to the `eigen-routes` plugin from [Part 2](/02-route-discovery-plugin):
...
This creates the standard framework data flow: first load gets data from serialized HTML, subsequent navigations fetch data from an API endpoint that calls the same loader functions on the server. This is exactly how [Next.js](https://nextjs.org)'s `getServerSideProps` works — subsequent navigations hit `/_next/data/...` to get fresh props.

---

## What to observe

1. **View source** — the HTML contains a `<script>` tag with `window.__EIGEN_DATA__` holding the serialized loader data. This is the same pattern as [Next.js](https://nextjs.org)'s `__NEXT_DATA__`.
...
3. **The loader never appears in the client bundle.** Because the client version of `eigen/routes` doesn't import loaders, Vite's tree-shaking eliminates them entirely. You can verify this in [`vite-plugin-inspect`](https://github.com/antfu-collective/vite-plugin-inspect) or by running a production build.
...
- [Vite Plugin API: `configureServer`](https://vite.dev/guide/api-plugin#configureserver) — The hook used to add the client-side data fetching endpoint.
