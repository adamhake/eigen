...
`configureServer` only exists during development. In production, there is no Vite dev server. The production server is a plain Express/Hono/Fastify app that you write separately (or that a deployment adapter generates).
</Callout>
