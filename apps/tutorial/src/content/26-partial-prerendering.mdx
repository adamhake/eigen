---
title: "Part 26: Partial Prerendering"
description: "Static shell generation at build time with Suspense boundaries as dynamic insertion points."
concepts:
...
```typescript title="plugins/eigen-ppr.ts"
async closeBundle() {
...
In Part 27, we'll build **AI streaming** — extending server functions to support structured streaming of AI model responses, with typed tool calls and progressive UI updates. This combines the server function pipeline from Part 15 with React's streaming architecture.
