---
title: "Part 12: Streaming SSR"
description: "renderToPipeableStream, Suspense-driven streaming, the shell vs content model, and streaming through Express."
---
...
sequenceDiagram
    ...
    Server-->>Browser: Shell HTML (nav, layout, fallbacks)
    Note over Browser: Renders immediately —<br/>user sees layout + spinners
    Server-->>Browser: <script> injects Stats panel
    Note over Browser: Stats appear, hydrate
    Server-->>Browser: <script> injects Orders table
    Note over Browser: Orders appear, hydrate
```
...
```typescript title="packages/eigen/resource.ts"
/**
...
async function start() {
  ...
  app.get('*', async (req, res) => {
    ...
    try {
      ...
      // Detect bots by user-agent (simplified — a production implementation
      // would use a library like `isbot` for comprehensive bot detection)
...
<Tabs items={["Node (renderToPipeableStream)", "Edge (renderToReadableStream)"]}>
<Tab value="Node (renderToPipeableStream)">

The Node.js streaming API uses `renderToPipeableStream`, which returns a pipeable Node stream. The `onShellReady` callback fires when the initial shell is ready to send, and `onAllReady` fires when all Suspense boundaries have resolved — useful for bots that need the complete page.

See the full implementation in the `renderStream` function above.

</Tab>
<Tab value="Edge (renderToReadableStream)">

If you're targeting Cloudflare Workers or Deno (the edge environment from Part 19), use `renderToReadableStream` instead — it returns a Web `ReadableStream` and uses `allReady` instead of the callback-based `onAllReady`:
